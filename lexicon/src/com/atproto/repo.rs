...
impl<'de> Deserialize<'de> for StrongRef {
        ...
        D: Deserializer<'de>,
    {
        ...
        let cid = Cid::try_from(helper.cid).map_err(serde::de::Error::custom)?;
        Ok(StrongRef {
            uri: helper.uri,
            cid,
        })
...
impl Serialize for OriginalBlob {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("OriginalBlob", 5)?;
        state.serialize_field("$type", &self.r#type)?;
        state.serialize_field("ref", &self.r#ref.as_ref().map(|c| c.to_string()))?;
...
impl<'de> Deserialize<'de> for OriginalBlob {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
