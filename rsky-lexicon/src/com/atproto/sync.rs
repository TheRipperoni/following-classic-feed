...
impl Serialize for SubscribeReposCommitOperation {
    ...
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("SubscribeReposCommitOperation", 3)?;
        state.serialize_field("path", &self.path)?;
        state.serialize_field("action", &self.action)?;
        state.serialize_field("cid", &self.cid.as_ref().map(|c| c.to_string()))?;
        state.end()
    }
}

impl<'de> Deserialize<'de> for SubscribeReposCommitOperation {
        ...
        D: Deserializer<'de>,
    {
        ...
        let cid = match helper.cid {
            Some(s) => Some(Cid::try_from(s).map_err(serde::de::Error::custom)?),
            None => None,
        };
        Ok(SubscribeReposCommitOperation {
            path: helper.path,
...
impl Serialize for SubscribeReposCommit {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("SubscribeReposCommit", 12)?;
        state.serialize_field("seq", &self.seq)?;
        state.serialize_field("time", &self.time)?;
        state.serialize_field("rebase", &self.rebase)?;
        state.serialize_field("tooBig", &self.too_big)?;
        state.serialize_field("repo", &self.repo)?;
        state.serialize_field("commit", &self.commit.to_string())?;
        state.serialize_field("prev", &self.prev.as_ref().map(|c| c.to_string()))?;
        state.serialize_field("rev", &self.rev)?;
        state.serialize_field("since", &self.since)?;
        state.serialize_field("blocks", &serde_bytes::Bytes::new(&self.blocks))?;
        state.serialize_field("ops", &self.ops)?;
        state.serialize_field("blobs", &self.blobs)?;
        state.end()
    }
}

impl<'de> Deserialize<'de> for SubscribeReposCommit {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        let helper = Helper::deserialize(deserializer)?;
        Ok(SubscribeReposCommit {
            seq: helper.seq,
            time: helper.time,
            rebase: helper.rebase,
            too_big: helper.too_big,
            repo: helper.repo,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SubscribeReposMigrate {
    pub seq: i64,
    pub did: String,
