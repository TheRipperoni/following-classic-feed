...
`jetstream` is a high-performance Rust service designed to subscribe to the AT Protocol [Jetstream](https://github.com/bluesky-social/jetstream). It consumes events from the firehose, processes repo commits, account updates, and identity changes, and forwards relevant data to a queuing service.
...
- **Modular Design**: Leverages `lexicon` for protocol-compliant data structures.

### Requirements

- **Rust**: Latest stable toolchain (v1.75+ recommended).
- **OpenSSL**: Required for secure WebSocket and HTTP connections.
- **Queuing Service**: A compatible service (like `feedgen`) to receive processed events.
...
docker run --env-file .env jetstream
```
