...
pub enum Lexicon {
    ...
    #[serde(rename(deserialize = "app.bsky.feed.threadgate"))]
...
mod tests {
    ...
    fn test_read_commit_create_profile() {
        ...
        match response {
            JetstreamRepoMessage::Commit(commit) => {
                assert_eq!(commit.commit.collection, "app.bsky.actor.profile");
                if let Some(Lexicon::AppBskyActorProfile(profile)) = &commit.commit.record {
    ...
    fn test_read_commit_create_threadgate() {
        ...
        match response {
            JetstreamRepoMessage::Commit(commit) => {
                assert_eq!(commit.commit.collection, "app.bsky.feed.threadgate");
